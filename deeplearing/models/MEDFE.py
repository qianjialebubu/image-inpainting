...
class MEDFE(BaseModel):
    ...
    def forward(self):
        ...
        # print("-------------------------")
        # hot_image(self.fake_out,0,True,'fake_ab_hot')
...
def hot_image(input,channel,isShowColorbar,output_name):
    x_DEE = input.cpu().detach()
    channel = channel
    selected_channel = x_DEE[:, channel, :, :]

    # 归一化特征图到 [0, 1] 范围
    # normalized_map = (selected_channel - np.min(selected_channel, axis=(2,3))) / (
    #             np.max(selected_channel,axis=(2,3)) - np.min(selected_channel, axis=(2,3)))

    # 绘制热力图
    plt.figure(figsize=(8, 8))
    plt.imshow(selected_channel[0], cmap='hot',interpolation='nearest')
    plt.axis('off')  # Remove the axes
    plt.tight_layout()  # Remove the surrounding whitespace
    if(isShowColorbar):
        plt.colorbar()  # 添加颜色条
